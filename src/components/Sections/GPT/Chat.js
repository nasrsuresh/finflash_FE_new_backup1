...
const Chat = ({ dropdownValue }) => {
  ...
  const sampleQuestions = [
    "Please summarize the management discussion and analysis.",
    "What are the key takeaways from the latest earnings call transcript?",
    "What are the major revenue drivers for the company?",
  ];

  const addToChat = (text) => {
    setShowSamples(false); // hide the sample questions
        ...
        <div className="flex flex-1 overflow-hidden overflow-y-auto flex-col">
          ...
          {chatList.map((e, i) =>
            e.type == "user_question" ? (
              <div
                key={i}
                className="w-full p-4 border-solid border-y-2 text-black border-y-gray-200"
              >
                Q{Math.floor(i / 2) + 1}
                {") "}
                {e.text}
              ...
              <div key={i} className="w-full p-5 bg-gray-50  text-black">
                {e.text.split("\n").map((t, j) => (
                  <p key={j}>{t}</p>
                ))}
              </div>
